...
export default function Catalog() {
  ...
  const updateFilter = (key, value) => {
    ...
    if (newVal) next.set(key, newVal); else next.delete(key);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 border text-sm font-body transition-colors ${filtersOpen || activeFiltersCount > 0 ? 'border-gold text-gold' : 'border-dark-border text-zinc-400 hover:border-zinc-600'}`}
            >
              <SlidersHorizontal size={14} />
              Фильтры
              {activeFiltersCount > 0 && (
                <span className="w-4 h-4 bg-gold text-dark-DEFAULT text-[10px] font-bold rounded-full flex items-center justify-center">{activeFiltersCount}</span>
              )}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
          {SORT_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => setFilters(prev => ({ ...prev, sort: opt.value }))}
              className={`whitespace-nowrap px-4 py-1.5 text-xs font-body border transition-all ${filters.sort === opt.value ? 'border-gold text-gold bg-gold/10' : 'border-dark-border text-zinc-500 hover:border-zinc-600'}`}>
              {opt.label}
            </button>
          ))}
        </div>
            ...
            <div className="sticky top-24 space-y-6">
                ...
                <div className="space-y-1">
                  {categories.map(cat => (
                    <button key={cat.slug} onClick={() => updateFilter('category', cat.slug)}
                      className={`flex items-center justify-between w-full text-left px-3 py-2 text-sm font-body transition-colors ${filters.category === cat.slug ? 'text-gold bg-gold/10 border-l-2 border-gold' : 'text-zinc-400 hover:text-white hover:bg-dark-muted'}`}>
                      {cat.name}
                ...
                <div className="space-y-1">
                  {BRANDS.map(brand => (
                    <button key={brand} onClick={() => updateFilter('brand', brand)}
                      className={`flex items-center gap-2 w-full text-left px-3 py-1.5 text-sm font-body transition-colors ${filters.brand === brand ? 'text-gold' : 'text-zinc-400 hover:text-white'}`}>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" placeholder="От" value={filters.min_price} onChange={e => setFilters(prev => ({ ...prev, min_price: e.target.value }))} className="input-dark text-xs py-2" />
                  <input type="number" placeholder="До" value={filters.max_price} onChange={e => setFilters(prev => ({ ...prev, max_price: e.target.value }))} className="input-dark text-xs py-2" />
                </div>
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {Array(9).fill(0).map((_, i) => <div key={i} className="aspect-square skeleton" />)}
              </div>
            ) : products.length === 0 ? (
